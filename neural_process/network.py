...
"""
Implementation from https://github.com/soobinseo/Attentive-Neural-Process (with minor edits).
"""
...
class LatentEncoder(nn.Module):
    ...
    """
    def __init__(self, num_hidden, num_latent, x_dim, y_dim):
...
class Decoder(nn.Module):
    ...
    def __init__(self, num_hidden, x_dim, y_dim):
        ...
        self.target_projection = Linear(x_dim, num_hidden)
        self.linears = nn.ModuleList([Linear(num_hidden * 3, num_hidden * 3, w_init='relu') for _ in range(3)])  # TODO: replace with nn.Sequential
        self.final_projection = Linear(num_hidden * 3, y_dim)
    ...
    def forward(self, r, z, target_x):
        ...
        hidden = t.cat([t.cat([r,z], dim=-1), target_x], dim=-1)  # TODO: replace with t.cat([r, z, target_x], dim=-1)
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, key, value, query):
        ...
        # Dropout - removed as it could mess things up
...
class AttentiveNeuralProcess(nn.Module):
    def __init__(self, x_dim, y_dim, num_hidden):
        """
        We will assume y_dim = 1 for now.
        """
        super(AttentiveNeuralProcess, self).__init__()
        self.latent_encoder = LatentEncoder(num_hidden, num_hidden, x_dim, y_dim)
        self.deterministic_encoder = DeterministicEncoder(num_hidden, num_hidden, x_dim, y_dim)
        self.decoder = Decoder(num_hidden, x_dim, y_dim)
        self.BCELoss = nn.BCELoss()
        self.std = nn.Parameter(t.tensor(1.))

    def forward(self, context_x, context_y, target_x, target_y=None):
        """
        Internally splits batch into single-element batches and runs network on each.
        Inefficient way to avoid problems with varying numbers of context/target points.
        """
        train = target_y is not None
        batch_y_pred = []
        batch_kl = 0 if train else None
        batch_loss = 0 if train else None
        if target_y is None:
            target_y = [None]*len(target_x)
        for args in zip(context_x, context_y, target_x, target_y):
            args = [arg.unsqueeze(0) if arg is not None else None for arg in args]
            y_pred, kl, loss = self._forward(*args)
            batch_y_pred.append(y_pred)
            if train:
                batch_kl += kl
                batch_loss += loss
        return batch_y_pred, batch_kl, batch_loss


    def _forward(self, context_x, context_y, target_x, target_y=None):
        """
        Let m be context set size, n be target set size.
        context_x: batch_size x m x x_dim
        context_y: batch_size x m x y_dim
        target_x: batch_size x n x x_dim
        """
        ...
        if target_y is not None:
            ...
            log_prob = t.distributions.Normal(y_pred, self.std).log_prob(target_y).sum()
